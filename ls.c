  ...
  if (format == long_format || print_block_size)
    {
      ...
      char *p = human_readable (total_blocks, buf + 1, human_output_opts|human_group_digits,
      ...
      if (format == long_format || print_block_size)
        {
          ...
          int len = mbswidth (human_readable (blocks, buf, human_output_opts|human_group_digits,
                                              ST_NBLOCKSIZE, output_block_size),
                              0);
          ...
          if (S_ISCHR (f->stat.st_mode) || S_ISBLK (f->stat.st_mode))
          ...
          else
            {
              ...
              int len = mbswidth (human_readable (size, buf,
                                                  file_human_output_opts|human_group_digits,
  ...
  if (print_block_size)
    {
      ...
      char const *blocks =
        (! f->stat_ok
         ? "?"
         : human_readable (ST_NBLOCKS (f->stat), hbuf, human_output_opts|human_group_digits,
                           ST_NBLOCKSIZE, output_block_size));
      int pad;
      ...
      char const *size =
        (! f->stat_ok
         ...
         : human_readable (unsigned_file_size (f->stat.st_size),
                           hbuf, file_human_output_opts|human_group_digits, 1,
...
print_file_name_and_frills (const struct fileinfo *f, size_t start_col)
{
  ...
  if (print_block_size)
    printf ("%*s ", format == with_commas ? 0 : block_size_width,
            ! f->stat_ok ? "?"
            : human_readable (ST_NBLOCKS (f->stat), buf, human_output_opts|human_group_digits,
                              ST_NBLOCKSIZE, output_block_size));
    ...
    len += 1 + (format == with_commas
                ? strlen (! f->stat_ok ? "?"
                          : human_readable (ST_NBLOCKS (f->stat), buf,
                                            human_output_opts|human_group_digits, ST_NBLOCKSIZE,
                                            output_block_size))
                : block_size_width);
